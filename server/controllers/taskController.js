function createTask(req, res) {
    ...
    if (new Date(dueDate) < new Date()) return res.status(400).send("Fälligkeitsdatum liegt in der Vergangenheit.");
    if (assignee && team) return res.status(400).send("Bitte entweder Assignee oder Team auswählen, nicht beides.");
